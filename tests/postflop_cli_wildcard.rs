...
fn wildcard_inspection_returns_every_reachable_private_combo_at_the_node() {
    let fixture = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/tiny_river_baseline.json");
    let mut request: serde_json::Value = serde_json::from_slice(
        &fs::read(fixture).expect("fixture should be readable"),
    )
    ...
    let response: serde_json::Value = serde_json::from_slice(&output.stdout)
        .expect("stdout should be one JSON response");
    ...
    assert!(!hands.contains_key("*"), "wildcard is a selector, not a hand key");
    for hand in hands.values() {
        let actions = hand["actions"].as_object().expect("each combo needs a strategy");
        assert!(!actions.is_empty(), "each reachable combo needs action frequencies");
        let total: f64 = actions
            .values()
            .map(|frequency| frequency.as_f64().expect("frequency should be numeric"))
            .sum();
        assert!((total - 1.0).abs() < 1e-5, "frequencies should sum to one, got {total}");
